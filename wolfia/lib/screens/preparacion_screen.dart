...
class _PreparacionScreenState extends State<PreparacionScreen> {
  ...
  Future<void> _editarObjetivo(Objetivo o) async {
    ...
    final tiempoMinCtrl = TextEditingController(text: '${o.tiempoMinimo}');
    final tiempoMaxCtrl =
        TextEditingController(text: o.tiempoMaximo?.toString() ?? '');
    final recomendacionesCtrl = TextEditingController(text: o.recomendaciones);
    int prioridad = o.prioridad;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  Expanded(
                    child: TextField(
                        controller: tiempoMinCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Tiempo mínimo (min)'),
                        keyboardType: TextInputType.number),
                  ),
                  const SizedBox(width: AppSpacing.sm),
                  Expanded(
                    child: TextField(
                        controller: tiempoMaxCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Tiempo máximo (opcional)'),
                        keyboardType: TextInputType.number),
                  ),
                ]),
                const SizedBox(height: AppSpacing.sm),
                ...
                TextField(
                    ...
                    keyboardType: TextInputType.number),
                if (o.estadoMental != null) ...[
                  const SizedBox(height: AppSpacing.sm),
                  Text('Último estado mental registrado: ${o.estadoMental}',
    ...
    if (ok == true) {
      ...
      final tMin = int.tryParse(tiempoMinCtrl.text.trim());
      if (tMin != null && tMin > 0) o.tiempoMinimo = tMin;
      o.tiempoMaximo = int.tryParse(tiempoMaxCtrl.text.trim());
      o.recomendaciones = recomendacionesCtrl.text.trim();
  ...
  Future<void> _agregarObjetivo() async {
    ...
    final tiempoMinCtrl = TextEditingController(text: '10');
    final tiempoMaxCtrl = TextEditingController();
            ...
            child: Column(
              ...
              children: [
                TextField(
                  controller: descCtrl,
                  decoration:
                      const InputDecoration(labelText: 'Ej: Negra = 108'),
                  autofocus: true,
                ),
                const SizedBox(height: AppSpacing.sm),
                Row(children: [
    ...
    if (ok == true && descCtrl.text.trim().isNotEmpty) {
      ...
      final tMin = int.tryParse(tiempoMinCtrl.text.trim()) ?? 10;
      final tMax = int.tryParse(tiempoMaxCtrl.text.trim());
      final obj = Objetivo(
        id: _uuid.v4(),
        preparacionId: widget.preparacionId,
  ...
  String _subtituloObjetivo(Objetivo o) {
    final partes = <String>[
      'Prioridad ${o.prioridad}/5',
      'Puntos: ${o.puntos} · ${o.puntosPorMinuto}/min',
      'Tiempo: ${o.tiempoMinimo}${o.tiempoMaximo != null ? '-${o.tiempoMaximo}' : '+'} min',
    ];
    if (o.recomendaciones.isNotEmpty) {
      partes.add('Recomendación: ${o.recomendaciones}');
    }
    if (o.estadoMental != null) {
      partes.add('Último estado mental: ${o.estadoMental}');
    }
    return partes.join(' · ');
  }
