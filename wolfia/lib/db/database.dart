...
class WolfiaDb {
    ...
    return openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 7) {
          ...
          await db
              .execute('ALTER TABLE objetivos ADD COLUMN tiempo_maximo INTEGER');
          await db.execute(
              "ALTER TABLE objetivos ADD COLUMN recomendaciones TEXT NOT NULL DEFAULT ''");
          await db
  ...
  Future<Objetivo?> getObjetivoPorId(String id) async {
    final rows =
        await (await database).query('objetivos', where: 'id = ?', whereArgs: [id]);
